...
def pfih(problem_instance):
    ...
    customers_to_serve = list(range(n+1))
...
def check_constraints(x, problem_instance):
    n = problem_instance['customer_number']
    v = problem_instance['vehicle_number']
    y = [[0 for _ in range(v)] for _ in range(n+1)]
    q = problem_instance['demand']
    a = [0] * (n + 1)
    W = [0] * (n + 1)
    s = problem_instance['service_duration']
    t = problem_instance['t']
    e = problem_instance['time_window_start']
    l = problem_instance['time_window_end']
