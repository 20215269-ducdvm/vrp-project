...
from .route import Route, compute_euclidean_distance
...
class CostEvaluator:
    ...
    def __init__(self, nodes: list[Node], capacity: int, run_parameters: dict[str, Any]):
        ...
        for node1 in nodes:
            ...
            for node2 in nodes:
                self._costs[node1.node_id][node2.node_id] = compute_euclidean_distance(node1, node2)
    ...
    def _get_nearest_neighbors(self, node: Node, nodes: list[Node]) -> list[Node]:
        ...
        sorted_nodes = sorted(
            nodes,
            key=lambda x: compute_euclidean_distance(x, node)
        )
    ...
    def is_feasible_capacity(self, capacity: int) -> bool:
