from datastructure import Node, Edge, VRPProblem, VRPSolution, CostEvaluator
...
def test_determine_edge_badness():
    ...
    problem = VRPProblem("CVRP", nodes, 3, )
    ...
    assert edge.value == 300

    edge = evaluator.get_and_penalize_worst_edge()
    assert edge == Edge(nodes[2], nodes[3])
    assert edge.value == 150

    edge = evaluator.get_and_penalize_worst_edge()
    assert edge == Edge(nodes[3], nodes[0])
    assert edge.value == 200
