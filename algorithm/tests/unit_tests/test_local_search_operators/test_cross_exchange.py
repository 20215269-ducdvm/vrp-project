...
def test_search_cross_exchanges_from():
    ...
    setattr(sys, "VRP_NO_TIME_WINDOWS", True)
    ...
    print()
    assert found_moves
    for found_move in sorted(found_moves):
        found_move.print_log()
    best_move = sorted(found_moves)[0]
    assert best_move.improvement == 2710
    ...
    assert found_moves
    best_move = sorted(found_moves)[0]
    assert best_move.improvement == 1800
    ...
    assert found_moves
    best_move = sorted(found_moves)[0]
    assert best_move.improvement == 1709
