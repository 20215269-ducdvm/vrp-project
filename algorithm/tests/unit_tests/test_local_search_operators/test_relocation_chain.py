...
from heuristics.kgls.local_search.operator_relocation_chain import search_relocation_chains, RelocationChain
...
def test_search_relocation_chains():
    ...
    # pm = PenaltyManager.init_from(problem)
    # evaluator.set_infeasible_penalty_weights(pm.penalties[0], pm.penalties[1])
    setattr(sys, 'VRP_NO_TIME_WINDOWS', True)  # Disable time windows for this test
    # optimal routes are D-1-2-3-D and D-4-5-6-D
    ...
    relocation_chains: list[RelocationChain] = search_relocation_chains(
        solution=solution,
    ...
    for relocation_chain in relocation_chains:
        relocation_chain.print_log()
