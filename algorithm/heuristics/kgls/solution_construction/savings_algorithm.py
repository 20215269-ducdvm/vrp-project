...
from datastructure import Node, CostEvaluator, VRPProblem, VRPSolution, Route
...
def compute_savings(
    ...
    # For each customer, only consider its nearest neighbors from cost_evaluator
    for node_1 in customers:
        for node_2 in cost_evaluator.get_neighborhood(node_1):
            # Avoid duplicate pairs (only process if node_1 < node_2 by some order)
            if node_1.node_id < node_2.node_id:  # Using object ID as a simple way to order nodes
                saving = (
...
def compute_weighted_savings(
    ...
    for saving in savings_list:
        node_1, node_2 = saving.from_node, saving.to_node
        weighted_saving = (saving.saving / max_saving) + ((node_1.demand + node_2.demand) / max_demand)
        weighted_savings_list.append(Saving(node_1, node_2, weighted_saving))
...
def clark_wright_parallel(
    ...
    for saving in savings_list:
        ...
        if node1 in cannot_be_extended or node2 in cannot_be_extended:
            ...
            prop_route = Route([vrp_instance.nodes[0], node1, node2, vrp_instance.nodes[0]], 0, vrp_instance.capacity)
            prop_route1 = prop_route.proposal(
                prop_route.before(0),  # depot
                [
                    prop_route.at(1),  # node1
                    prop_route.at(2)  # node2
                ],
                prop_route.after(3)  # depot
            )
            if prop_route.is_feasible or prop_route1.is_feasible:
                if prop_route.is_feasible:
                    solution.add_route(
                        [node1, node2]
                    )
                else:
