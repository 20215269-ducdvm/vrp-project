...
class VRPSolution:
    ...
    def get_solution_penalties(self):
        distance = 0
        excess_load = 0
        time_warp = 0
        for route in self.routes:
        ...
        return distance, excess_load, time_warp

    @staticmethod
    def get_route_penalties(route: Route):
        """
        Returns the penalty costs of a given route.
        """
        distance = route.total_distance
        excess_load = route.excess_load
        time_warp = route.time_warp
    ...
    def is_feasible(self):
        """
        Checks if the solution is feasible based on the routes.
        """
        return True if [
            route.is_feasible() for route in self.routes
        ] else False
    ...
    def to_file(self, cost, path_to_file: str):
        with open(path_to_file, 'w') as file:
            route_id = 1
            for _, route in enumerate(self.routes, 1):
                if route.size > 0:
                    customer_ids = ' '.join(str(node.node_id) for node in route.customers)
                    file.write(f"Route #{route_id}: {customer_ids}\n")
                    route_id += 1
    ...
    def rearrange_route(self, route: Route, node_order: list[Node]):
