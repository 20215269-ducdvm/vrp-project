...
class DurationSegment:
    ...
    def merge(edge_duration: Duration,
        ...
        # at the second segment early.
        diff_wait = Duration(max(Duration(0), Duration(second._tw_early - at_second - first._tw_late)))

        # New twLate for the second segment. second_late is the last possible time to serve the first client in the second
        # segment to not violate the time window constraints.
